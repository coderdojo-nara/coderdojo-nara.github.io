...
### 先に読んでください（繰り返しフィールドの作り方）

`blog` の `gallery`、`next-event` の `badges`、`supporters` の `companies` は
**繰り返しフィールド**です。これには microCMS 特有の手順があります。

**1. 「繰り返し」は API 作成ウィザードには出てきません**

API を新規作成する画面のフィールド種類には「繰り返し」「カスタム」がありません。
API を作り終えたあと、**API 設定 → API スキーマ**の画面でフィールドを追加・編集すると選べます。
そのため、各 API はまず繰り返しフィールド**以外**を定義して作成し、あとから追加する流れになります。

**2. 繰り返す中身（カスタムフィールド）を先に作る必要があります**

繰り返しフィールドは「カスタムフィールドを繰り返すブロック」です。
対象のカスタムフィールドが存在しないと、繰り返しフィールドの参照先を指定できません。
カスタムフィールドは**コンテンツ一覧画面の右上「カスタムフィールド」**から作成します。

したがって、繰り返しフィールドを含む API は次の順で作業します。

1. 繰り返しフィールド以外を定義して API を作成する
2. コンテンツ一覧画面の右上「カスタムフィールド」から、繰り返す中身のカスタムフィールドを作成する
3. API 設定 → API スキーマ に戻り、種類「繰り返し」でフィールドを追加し、2 のカスタムフィールドを参照先に指定する

以下の各節では、この順序に合わせて**カスタムフィールドの定義を先に**書いています。

なお、カスタムフィールド自体のフィールド ID（`gallery-image` など）はコード側から参照しないため、
分かりやすい名前であれば変えても構いません。**中身のフィールド ID は表記どおりに**設定してください。
...
**カスタムフィールド「ギャラリー画像」**（フィールド ID: `gallery-image`）

`gallery` から参照します。先に作成してください。
...
| `gallery`     | 画像ギャラリー | 繰り返し                   |      | 参照先「ギャラリー画像」。frontmatter の `images`。本文の下に格子状で並ぶ |

`gallery` は API 作成後に API スキーマ画面から追加します（「先に読んでください」参照）。
...
**カスタムフィールド「バッジ」**（フィールド ID: `badge`）

`badges` から参照します。先に作成してください。
...
| `badges`      | バッジ      | 繰り返し           |      | 参照先「バッジ」。「参加費 無料」などの表示                 |

`badges` は API 作成後に API スキーマ画面から追加します（「先に読んでください」参照）。
...
**カスタムフィールド「サポート企業」**（フィールド ID: `company`）

`companies` から参照します。先に作成してください。
...
| `companies`   | 企業一覧 | 繰り返し           | ○    | 参照先「サポート企業」。0 件のときはセクションごと非表示 |

`companies` は API 作成後に API スキーマ画面から追加します（「先に読んでください」参照）。
